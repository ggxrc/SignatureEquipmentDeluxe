...
namespace Progression.Common.Projectiles
{
    /// <summary>
    ...
    public class SignatureGlobalProjectile : GlobalProjectile
    {
        ...
        private void ApplyProjectileStats(Projectile projectile, bool isDirectSpawn)
        {
            ...
            var gameplayConfig = ModContent.GetInstance<Configs.GameplayConfig>();
            var advancedConfig = ModContent.GetInstance<Configs.AdvancedConfig>();
            var scalingConfig = ModContent.GetInstance<Configs.ScalingConfig>();
            ...
            if (isDirectSpawn && !projectile.IsMinionOrSentryRelated && scalingConfig.AdditionalProjectileChanceIncrement)
            {
                SpawnAdditionalProjectiles(projectile, globalItem, scalingConfig, advancedConfig, gameplayConfig, false);
            }
        }
        ...
        /// </summary>
        private void SpawnAdditionalProjectiles(Projectile projectile, GlobalItems.SignatureGlobalItem globalItem, Configs.ScalingConfig scalingConfig, Configs.AdvancedConfig advancedConfig, Configs.GameplayConfig gameplayConfig, bool isMinion)
        {
            // Verifica blacklist do item
            if (scalingConfig.WeaponAdditionalProjectileChance.ItemBlackList.Contains(new ItemDefinition(ProjectileCreatorItem.type)))
                return;

            // Calcula a chance com HardCap do item se existir
            float garantChance;
            if (scalingConfig.WeaponAdditionalProjectileChance.ItemHardCap.TryGetValue(new ItemDefinition(ProjectileCreatorItem.type), out var itemCap))
            {
            ...
            int minRad = isMinion ? gameplayConfig.AdditionalProjectileMinRadMinion : gameplayConfig.AdditionalProjectileMinRad;
            int maxAdditionalRad = isMinion ? gameplayConfig.AdditionalProjectileMaxRadMinion : gameplayConfig.AdditionalProjectileMaxRad;
            float shootsToMaxRad = gameplayConfig.ProjectilesToMaxRad;
            ...
            if (advancedConfig.DebugMode && countShoots > 0)
        ...
        {
            var advancedConfig = ModContent.GetInstance<Configs.AdvancedConfig>();
            bool debug = advancedConfig != null && advancedConfig.DebugMode;
