...
namespace Progression.Common.Systems
{
    ...
    public class KillStreakSystem : ModPlayer
    {
        ...
        private Configs.ProgressionConfig progressionConfig;
        ...
        {
            progressionConfig ??= ModContent.GetInstance<Configs.ProgressionConfig>();
            clientConfig ??= ModContent.GetInstance<Configs.ClientConfig>();
            
            if (!progressionConfig.EnableKillStreakSystem)
                return;
        ...
        public float GetStreakXPMultiplier()
        {
            ...
            if (!progressionConfig.EnableKillStreakSystem || !progressionConfig.EnableStreakXPBonus)
                return 1f;
            
            // +1% por kill de streak, até o máximo configurado
            float bonusPercent = KillStreak * progressionConfig.StreakXPBonusPerKill;
            bonusPercent = Math.Min(bonusPercent, progressionConfig.StreakXPBonusMax);
